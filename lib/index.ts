...
class Y18N {
  ...
  __(...args: (string | Function)[]): string {
    if (typeof args[0] !== 'string') {
      return this._taggedLiteral(args[0] as any, ...(args as any));
    }
    ...
    return shim.format.apply(
      shim.format,
      [this.cache[this.locale][str] || str].concat(args as string[])
    );
  }
  ...
  _processWriteQueue() {
    ...
    shim.fs.writeFile(languageFile, serializedLocale, 'utf-8', (err: Error) => {
